...
from utils.options import StegoConfiguration, TrainingOptions
...
def save_images(original_images, watermarked_images, epoch, folder, resize_to=None):
    """
    Saves a comparison image showing original and watermarked images stacked vertically.

    :param original_images: torch tensor of shape (batch_size x channels x height x width)
    :param watermarked_images: torch tensor of shape (batch_size x channels x height x width)
    :param epoch: current epoch number
    :param folder: folder to save the image to
    :param resize_to: if not None, the images are resized to this size before saving
    """
    ...
    torchvision.utils.save_image(stacked_images, filename, nrow=8, normalize=False)
...
def last_checkpoint_from_folder(folder: str):
    """ Returns the last checkpoint file from the given folder """
    last_file = sorted_nicely(os.listdir(folder))[-1]
    last_file = os.path.join(folder, last_file)
    # last_file = os.path.join(folder, "no_noise--epoch-10.pyt")
    return last_file


def save_checkpoint(model: Stego, experiment_name: str, epoch: int, checkpoint_folder: str):
...
def load_options(options_file_name) -> (TrainingOptions, StegoConfiguration, dict):
...
def get_data_loaders(hidden_config: StegoConfiguration, train_options: TrainingOptions):
    ...
    train_images = data.Subset(train_images, np.random.choice(len(train_images), 10000, replace=False))
    ...
    validation_images = data.Subset(validation_images, np.random.choice(len(validation_images), 1000, replace=False))
...
def create_folder_for_run(runs_folder, experiment_name):
    """ Creates a folder for the current run inside the given runs_folder. Also updates the config.json file"""
    ...
    with open('./utils/config.json', 'r') as f:
        config = json.load(f)
    config['stego-folder'] = this_run_folder
...
def write_losses(file_name, losses_accu, epoch, duration):
    """ Appends the average losses for the epoch to a CSV file. If the file does not exist, it creates it and writes the header first."""
